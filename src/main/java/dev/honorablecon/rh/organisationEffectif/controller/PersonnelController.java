...
import dev.honorablecon.rh.organisationEffectif.model.input.PersonnelInput;
...
public class PersonnelController {
    ...
    public List<Personnel> getAllPersonnels(){
        return personnelRepository.findAllByDeletedIsFalseOrDeletedIsNull();
    }
    ...
    @MutationMapping(name = "deletePersonnel")
    @Transactional()
    public Personnel deletePersonnel(@Argument Long id){
        Personnel personnel = personnelRepository.findById(id)
                                                 .orElseThrow();
        personnel.setDeleted(true);
        personnel.getContrats().forEach(contrat -> contrat.setEtat(false));
        personnel.getIntegrationFilieres().forEach(integrationFiliere -> integrationFiliere.setEtat(false));
